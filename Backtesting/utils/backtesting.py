...
def df_backtesting(df, ticker, investment, risk_free_rate):

    global df_trades

    df['Previous_day_close'] = df['Adj Close'].shift(1)

    df_position = uf.get_df_trade_position(df, 'Position', 'Signal')
    ...
    roi =  pm.get_return_on_investment(df_trades, investment)
