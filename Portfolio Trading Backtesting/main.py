...
list_stock = ws.get_stock_list(filter_= "Uptrend")
...
# list_strategy = ['sma_crossover','rsi_range','rsi_crossover','support_resistance_breakout','macd_crossover','mean_reversion_with_rsi']
list_strategy = ['mean_reversion_with_rsi']
    ...
    if strategy == 'sma_crossover':
        for short_sma in list_short_sma:
            for long_sma in list_long_sma: 
                ...
                for ticker in list_stock :
                    try:
                        df_raw = uf.getdata(ticker, close = "Adj Close", n_days=60, frequency="5m")       
        ...
        for overbought, oversold in zip(list_overbought, list_oversold): 
            ...
            for ticker in list_stock:   
                try:
                    df_raw = uf.getdata(ticker, close = "Adj Close", n_days=60, frequency="5m")  
                    df = mean_reversion_with_rsi.get_signal(df_raw, 'Adj Close', [50,200], rsi_range_period, oversold, overbought, stop_loss_pct=0.03)
                    df_trades, df_heatmap =bt.df_backtesting(df, ticker, risk_free_rate)
                except Exception as e:
                    print(f"Error occurred: {e}")
                    continue
        ...
        for ticker in list_stock:   
            try:
                df_raw = uf.getdata(ticker, close = "Adj Close",n_days=60, frequency="5m")  
                df = macd_crossover.get_signal(df_raw, 'Adj Close', 0.03, 60, 6, 10) 
                df_trades, df_heatmap =bt.df_backtesting(df, ticker, risk_free_rate)
            except Exception as e:
                print(f"Error occurred: {e}")
                continue
